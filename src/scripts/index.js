...
function renderPictures(list) {
  ...
  list.forEach(function(element) {
    const link = clone.querySelector("a").cloneNode(true);
    link.href = element.url;
    link.dataset.id = element.id;

    const image = link.querySelector("img");
    image.src = cropImage(element.download_url, 5);
    // По хорошему в альт атрибуте должно быть кратко описано что изображено на картинке, а не имя фотографа.
 ...
 */
function renderPopupPicture(picture) {
...
const imageHandler = function(evt) {
  ...
  if (parentLink) {
    getPictureInfo(parentLink.dataset.id);
  }
};
